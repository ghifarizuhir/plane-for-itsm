...
class TestMagicLinkGenerate:
    ...
    def setup_user(self, db):
        ...
        user = User.objects.create(email="user@terraline.space")
    ...
    def test_magic_generate(self, mock_magic_link, api_client, setup_user, setup_instance):
        ...
        ri = redis_instance()
        ri.delete("magic_user@terraline.space")

        response = api_client.post(url, {"email": "user@terraline.space"}, format="json")
        ...
        assert args[0] == "user@terraline.space"  # First arg should be the email
    ...
    def test_max_generate_attempt(self, mock_magic_link, api_client, setup_user, setup_instance):
        ...
        for _ in range(4):
            api_client.post(url, {"email": "user@terraline.space"}, format="json")
...
class TestSignInEndpoint:
    ...
    def test_password_validity(self, django_client, setup_user, setup_instance):
        ...
        url = reverse("sign-in")
        response = django_client.post(url, {"email": "user@terraline.space", "password": "user123"}, follow=True)
    ...
    def test_user_login(self, django_client, setup_user, setup_instance):
        ...
        response = django_client.post(url, {"email": "user@terraline.space", "password": "user@123"}, follow=False)
    ...
    def test_next_path_redirection(self, django_client, setup_user, setup_instance):
        ...
        response = django_client.post(
            url,
            {"email": "user@terraline.space", "password": "user@123", "next_path": next_path},
            follow=False,
        )
...
class TestMagicSignIn:
    ...
    def test_expired_invalid_magic_link(self, django_client, setup_user, setup_instance):
        ...
        url = reverse("magic-sign-in")
        response = django_client.post(url, {"email": "user@terraline.space", "code": "xxxx-xxxxx-xxxx"}, follow=False)
    ...
    def test_user_does_not_exist(self, django_client, setup_instance):
        ...
        response = django_client.post(
            url,
            {"email": "nonexistent@terraline.space", "code": "xxxx-xxxxx-xxxx"},
            follow=True,
        )
    ...
    def test_magic_code_sign_in(self, mock_magic_link, django_client, api_client, setup_user, setup_instance):
        ...
        response = api_client.post(gen_url, {"email": "user@terraline.space"}, format="json")
        ...
        ri = redis_instance()
        user_data = json.loads(ri.get("magic_user@terraline.space"))
        token = user_data["token"]

        # Use Django client to test the redirect flow without following redirects
        url = reverse("magic-sign-in")
        response = django_client.post(url, {"email": "user@terraline.space", "code": token}, follow=False)
    ...
    def test_magic_sign_in_with_next_path(self, mock_magic_link, django_client, api_client, setup_user, setup_instance):
        ...
        response = django_client.post(
            url,
            {"email": "user@terraline.space", "code": token, "next_path": next_path},
            follow=False,
        )
...
class TestMagicSignUp:
    ...
    def test_user_already_exists(self, django_client, db, setup_instance):
        ...
        User.objects.create(email="existing@terraline.space")

        url = reverse("magic-sign-up")
        response = django_client.post(url, {"email": "existing@terraline.space", "code": "xxxx-xxxxx-xxxx"}, follow=True)
    ...
    def test_expired_invalid_magic_link(self, django_client, setup_instance):
        ...
        url = reverse("magic-sign-up")
        response = django_client.post(url, {"email": "new@terraline.space", "code": "xxxx-xxxxx-xxxx"}, follow=False)
    ...
    def test_magic_sign_up_with_next_path(self, mock_magic_link, django_client, api_client, setup_instance):
        ...
        email = "newuser2@terraline.space"
...
class TestMagicSignInVerifyAttempts:
    ...
    EMAIL = "verify-attempts@terraline.space"
...
class TestMagicSignUpVerifyAttempts:
    ...
    EMAIL = "signup-verify-attempts@terraline.space"
...
class TestAuthenticationThrottle:
    ...
    def test_magic_sign_in_throttled(self, django_client, setup_instance):
        ...
        with patch.object(AuthenticationThrottle, "rate", "2/minute"):
            for _ in range(2):
                response = django_client.post(url, {"email": "throttle@terraline.space", "code": "000000"}, follow=False)
                assert response.status_code == 302
    ...
    def test_magic_sign_up_throttled(self, django_client, setup_instance):
        ...
        with patch.object(AuthenticationThrottle, "rate", "1/minute"):
            response = django_client.post(url, {"email": "throttle-up@terraline.space", "code": "000000"}, follow=False)
            assert "RATE_LIMIT_EXCEEDED" not in response.url
    ...
    def test_password_sign_in_throttled(self, django_client, setup_instance):
        ...
        with patch.object(AuthenticationThrottle, "rate", "2/minute"):
            for _ in range(2):
                response = django_client.post(
                    url, {"email": "throttle@terraline.space", "password": "secret123"}, follow=False
                )
                assert response.status_code == 302
                assert "RATE_LIMIT_EXCEEDED" not in response.url

            # The 3rd request from the same IP within the window trips the throttle.
            response = django_client.post(url, {"email": "throttle@terraline.space", "password": "secret123"}, follow=False)
            assert response.status_code == 302
            assert "RATE_LIMIT_EXCEEDED" in response.url
    ...
    def test_password_sign_up_throttled(self, django_client, setup_instance):
        ...
        url = reverse("sign-up")
        with patch.object(AuthenticationThrottle, "rate", "1/minute"):
            response = django_client.post(url, {"email": "throttle-up@terraline.space", "password": "secret123"}, follow=False)
            assert "RATE_LIMIT_EXCEEDED" not in response.url
...
class TestBotUserLoginBlocked:
    ...
    BOT_EMAIL = "bot-login@terraline.space"
    HUMAN_EMAIL = "human-login@terraline.space"
...
class TestBotUserAdminSignInBlocked:
    ...
    BOT_EMAIL = "admin-bot@terraline.space"
    PASSWORD = "Str0ng-Pass!42"
