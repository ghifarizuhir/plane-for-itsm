...
| 2   | [`02-data-model.md`](./02-data-model.md)             | ✅ Draft | 01         | Postgres per-table (skema Django, dilayani Rust) + ITSM extensions    |
| 3   | [`03-api-contract.md`](./03-api-contract.md)         | ✅ Draft | 01, 02     | Axum + X-Api-Key/Bearer + pagination/filter/sort/error parity         |
| 4   | [`04-design-system.md`](./04-design-system.md)       | ✅ Draft | —          | @plane/ui + tailwind-config + tokens + icon                           |
| 5   | [`05-testing-strategy.md`](./05-testing-strategy.md) | ✅ Draft | 01         | cargo test + gates + pytest baseline + Vitest + Playwright (deferred) |
| 6   | [`06-ops-runbook.md`](./06-ops-runbook.md)           | ✅ Draft | 01, 02     | docker-compose + env + deploy + backup                                |

> Terra punya 13 doc (`terra/docs/design/README.md:11` — termasuk 01-erd, 09-realtime, 12-mcp, 13-release-versioning). Plane-for-itsm **ramping jadi 6** — yang tidak relevan (MCP, erd `entities` JSONB) dihapus; yang Plane-specific (Rust Axum, MobX, Live) digabung di `01-architecture`.
...
| Backend     | **Rust Axum + SQLx + Redis Stream** (`apps/api-rs`, tag `rust-cutover-v1`); skema Django = referensi, fallback `api-legacy` | 01, 02 |
...
| Auth        | **X-Api-Key + Bearer** (Rust `middleware/auth.rs`); session/cookie + OAuth social via Django fallback                       | 03     |
| ITSM model  | Postgres per-table (bukan `entities` JSONB Terra) — `Incident`/`Problem`/etc. via IssueType atau tabel baru                 | 02     |
...
| 2026-09-05 | cutover `rust-cutover-v1`: backend/auth/testing → Rust |
