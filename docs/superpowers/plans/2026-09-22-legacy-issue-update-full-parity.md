...
13. **Nullish/edge laxness inherited from the create slice:** `""` is treated as an explicit clear for the nullable id fields (`state_id`/`parent_id`/`estimate_point`/`type_id`) where DRF 400s (`de_double_opt_*_lax`, `issue_common.rs`); `description: null` → 400 while Django ignores the unknown `description` key entirely (its own key is `description_json`); and the `parent`/`estimate_point` error strings are the create-slice wording (`"parent is not valid"`, `"estimate_point is not valid"`) rather than Django's longer messages (`serializers/issue.py:186,195`).
